...
export function sheetCsvToTemples(csv) {
  ...
  return rows
    ...
    .filter((record) => record["노출 여부"].toUpperCase() === "ON")
    .map((record) => ({
      id: record.templeId,
      name: record["교당명"],
      aliases: unique([
      ...
      address: record["주소"],
      phone: record["전화 표시"].toUpperCase() === "ON" ? record["전화번호"] : "",
      youtube: record["영상 사용"].toUpperCase() === "ON" ? record["YouTube URL"] : "",
      blog: record["블로그 사용"].toUpperCase() === "ON" ? record["블로그 URL"] : "",
      naverPlaceUrl: record["네이버 플레이스 URL"],
      kakaoMapUrl: record["카카오맵 URL"],
      naverMapUrl: record["네이버지도 URL"],
